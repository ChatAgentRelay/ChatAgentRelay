...
export class EncryptionEngine {
  ...
  private async getKey(): Promise<CryptoKey> {
    ...
    this.key = await crypto.subtle.importKey("raw", keyData, { name: ALGORITHM }, false, ["encrypt", "decrypt"]);
    return this.key;
  }
  ...
  async encryptFields(obj: Record<string, unknown>, fields: string[]): Promise<Record<string, unknown>> {
    const result = { ...obj };
  ...
  async decryptFields(obj: Record<string, unknown>, fields: string[]): Promise<Record<string, unknown>> {
    const result = { ...obj };
    for (const field of fields) {
      const val = result[field];
