...
import type { ChannelType, ConfigStore, RouteEngine } from "@chat-agent-relay/config-store";
...
function sseResponse(handler: (send: (event: WebChatStreamEvent) => void, close: () => void) => void): Response {
    ...
    async fetch(req) {
      try {
        ...
        if (path === "/api/agents" && method === "POST") {
          ...
          try {
            const record = await configDb.addAgent(
              name as string,
              type as "a2a",
              (agentConfig ?? {}) as Record<string, unknown>,
            );
            await agentRegistry.register(record);
            return jsonResponse({ ...record, config: maskConfig(record.type, record.config) }, 201);
          } catch (err) {
        ...
        if (path === "/api/channels" && method === "POST") {
          ...
          try {
            const record = await configDb.addChannel(
              name as string,
              type as ChannelType,
        ...
        if (slackWebhookPath && method === "POST") {
          ...
          const signingSecret = await configDb
            .getChannel(slackChannel.name)
            .then((record) =>
              typeof record?.config["signingSecret"] === "string" ? record.config["signingSecret"] : undefined,
            );
        ...
        if (path === "/api/teams/messages" && method === "POST") {
          ...
          const verification = await verifyWebhookRequest(req, async (request) => {
        ...
        if (path === "/api/telegram/webhook" && method === "POST") {
          ...
          const secretToken =
            typeof telegramConfig?.config["secretToken"] === "string"
        ...
        if (path === "/api/lark/webhook" && method === "POST") {
          ...
          const encryptKey =
            typeof larkConfig?.config["encryptKey"] === "string" ? larkConfig.config["encryptKey"] : undefined;
        ...
        if (path === "/api/dingtalk/webhook" && method === "POST") {
          ...
          const secret =
            typeof dingtalkConfig?.config["secret"] === "string" ? dingtalkConfig.config["secret"] : undefined;
        ...
        if (path === "/api/whatsapp/webhook" && method === "GET") {
          ...
          const verifyToken =
            typeof whatsappConfig?.config["verifyToken"] === "string"
        ...
        if (path === "/api/whatsapp/webhook" && method === "POST") {
          ...
          const whatsappConfig = await configDb.getChannel(whatsappChannel.name);
          const appSecret =
            typeof whatsappConfig?.config["appSecret"] === "string" ? whatsappConfig.config["appSecret"] : undefined;
        ...
        return errorResponse(isDecryptionError ? `Config error: ${msg}` : "Internal server error", 500);
      }
    },
  });
