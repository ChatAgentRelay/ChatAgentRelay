...
async function channelAdd(positional: string[], flags: Record<string, string>): Promise<void> {
  ...
  if (!name) {
    err("Usage: car channel add <name> --type=slack|discord|webchat|telegram|lark|dingtalk|teams|whatsapp [options]");
    return;
  }

  const validChannelTypes = ["slack", "discord", "webchat", "telegram", "lark", "dingtalk", "teams", "whatsapp"];
  const type = (flags["type"] ??
    (await prompt("Channel type (slack/discord/webchat/telegram/lark/dingtalk/teams/whatsapp): "))) as ChannelType;
  if (!validChannelTypes.includes(type)) {
    err(`Invalid type: ${type}. Valid: ${validChannelTypes.join(", ")}`);
    return;
  }
  ...
  if (type === "slack") {
    ...
    if (!config["botToken"] || !config["appToken"]) {
      err("Slack requires --bot-token and --app-token");
      return;
    }
  } else if (type === "discord") {
    config["botToken"] = flags["bot-token"] ?? (await prompt("Bot Token: "));
    if (!config["botToken"]) {
      err("Discord requires --bot-token");
      return;
    }
    ...
    if (!config["botToken"]) {
      err("Telegram requires --bot-token");
      return;
    }
    ...
    if (!config["appId"] || !config["appSecret"]) {
      err("Lark requires --app-id and --app-secret");
      return;
    }
...
async function channelShow(positional: string[]): Promise<void> {
  const name = positional[2];
  if (!name) {
    err("Usage: car channel show <name>");
    return;
  }
  const db = openDb();
  try {
    ...
    for (const [k, v] of Object.entries(ch.config)) {
      const display =
        typeof v === "string" && (k.toLowerCase().includes("token") || k.toLowerCase().includes("key"))
          ? mask(v)
          : String(v);
      out(`  ${k}: ${display}`);
    }
...
async function channelToggle(positional: string[], enable: boolean): Promise<void> {
  const name = positional[2];
  if (!name) {
    err(`Usage: car channel ${enable ? "enable" : "disable"} <name>`);
    return;
  }
  const port = getApiPort();
  if (await isServerRunning(port)) {
...
async function channelRemove(positional: string[]): Promise<void> {
  const name = positional[2];
  if (!name) {
    err("Usage: car channel remove <name>");
    return;
  }
  const port = getApiPort();
  if (await isServerRunning(port)) {
...
async function agentAdd(positional: string[], flags: Record<string, string>): Promise<void> {
  const name = positional[2];
  if (!name) {
    err("Usage: car agent add <name> --endpoint=URL [options]");
    return;
  }
...
async function agentShow(positional: string[]): Promise<void> {
  ...
  try {
    ...
    for (const [k, v] of Object.entries(ag.config)) {
      const display =
        typeof v === "string" && (k.toLowerCase().includes("key") || k.toLowerCase().includes("token"))
          ? mask(v)
          : String(v);
      out(`  ${k}: ${display}`);
    }
...
async function agentToggle(positional: string[], enable: boolean): Promise<void> {
  const name = positional[2];
  if (!name) {
    err(`Usage: car agent ${enable ? "enable" : "disable"} <name>`);
    return;
  }
  const port = getApiPort();
  if (await isServerRunning(port)) {
...
async function agentRemove(positional: string[]): Promise<void> {
  const name = positional[2];
  if (!name) {
    err("Usage: car agent remove <name>");
    return;
  }
  const port = getApiPort();
  if (await isServerRunning(port)) {
...
async function routeAdd(flags: Record<string, string>): Promise<void> {
  ...
  if (!agentName) {
    err("Usage: car route add --agent=<name> [--match=channel:<ch>|pattern:<re>] [--default] [--priority=N]");
    return;
  }
  ...
  if (flags["default"] !== undefined) {
    ...
    if (colonIdx < 0) {
      err("--match must be channel:<name> or pattern:<regex>");
      return;
    }
    ...
    if (kind === "channel") matchType = "channel";
      ...
      err(`Unknown match type: ${kind}`);
      return;
    }
  } else {
    err("Specify --match=channel:<name>, --match=pattern:<regex>, or --default");
    return;
  }

  const port = getApiPort();
  if (await isServerRunning(port)) {
    await apiCall(port, "POST", "/api/routes", {
...
async function routeList(): Promise<void> {
  ...
  if (routes.length === 0) {
    out("No routes configured.");
    return;
  }
  out("ID   PRI  TYPE      MATCH              AGENT            ON");
  for (const r of routes) {
    out(
      `${String(r.id).padEnd(5)}${String(r.priority).padEnd(5)}${r.match_type.padEnd(10)}${(r.match_value ?? "-").padEnd(19)}${r.agent_name.padEnd(17)}${r.enabled ? "yes" : "no"}`,
    );
  }
}
...
async function routeToggle(positional: string[], enable: boolean): Promise<void> {
  ...
  if (!id) {
    err(`Usage: car route ${enable ? "enable" : "disable"} <id>`);
    return;
  }
  const port = getApiPort();
  if (await isServerRunning(port)) {
...
function configSet(positional: string[]): void {
  ...
  if (!key || value === undefined) {
    err("Usage: car config set <key> <value>");
    return;
  }
  const db = openDb();
  try {
...
async function status(): Promise<void> {
  ...
  if (await isServerRunning(port)) {
    try {
      ...
      const agents = (await apiCall(port, "GET", "/api/agents")) as Array<{
        name: string;
        type: string;
        enabled: boolean;
      }>;
      ...
      const channels = (await apiCall(port, "GET", "/api/channels")) as Array<{
        name: string;
        type: string;
        enabled: boolean;
      }>;
  ...
  try {
    switch (cmd) {
      ...
      case "channel":
        switch (sub) {
          case "add":
            await channelAdd(positional, flags);
            break;
          case "list":
            await channelList();
            break;
          case "show":
            await channelShow(positional);
            break;
          case "enable":
            await channelToggle(positional, true);
            break;
          case "disable":
            await channelToggle(positional, false);
            break;
          case "remove":
            await channelRemove(positional);
            break;
          default:
            err("Usage: car channel add|list|show|enable|disable|remove");
            break;
        }
        break;
      case "agent":
        switch (sub) {
          case "add":
            await agentAdd(positional, flags);
            break;
          case "list":
            await agentList();
            break;
          case "show":
            await agentShow(positional);
            break;
          case "enable":
            await agentToggle(positional, true);
            break;
          case "disable":
            await agentToggle(positional, false);
            break;
          case "remove":
            await agentRemove(positional);
            break;
          default:
            err("Usage: car agent add|list|show|enable|disable|remove");
            break;
        }
        break;
      case "route":
        switch (sub) {
          case "add":
            await routeAdd(flags);
            break;
          case "list":
            await routeList();
            break;
          case "enable":
            await routeToggle(positional, true);
            break;
          case "disable":
            await routeToggle(positional, false);
            break;
          case "remove":
            await routeRemove(positional);
            break;
          default:
            err("Usage: car route add|list|enable|disable|remove");
            break;
        }
        break;
      case "config":
        switch (sub) {
          case "set":
            configSet(positional);
            break;
          case "get":
            configGet(positional);
            break;
          case "list":
            configList();
            break;
          default:
            err("Usage: car config set|get|list");
            break;
        }
        break;
      case "status":
