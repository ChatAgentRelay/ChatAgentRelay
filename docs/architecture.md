...
This document summarizes how CAP sits between chat platforms and agent runtimes, how canonical events move through the stack, and how the workspace packages depend on each other. For normative contracts and behavior, see [reference-architecture.md](rfcs/architecture/reference-architecture.md), [channel-adapter-interface-spec.md](rfcs/adapters/channel-adapter-interface-spec.md), and [backend-adapter-interface-spec.md](rfcs/adapters/backend-adapter-interface-spec.md).
...
CAP defines a small vocabulary of **canonical events**: JSON documents with a shared envelope, specialized payloads per `event_type`, and causal linkage via `correlation_id` and `causation_id`. The **happy path** for one user turn is a fixed seven-event chain:
...
Every event type is validated against JSON Schemas loaded by `@cap/contract-harness`. The frozen fixture corpus in `docs/schemas/fixtures/` is the machine-readable baseline for implementations.
...
The following sequence is the logical order of handoffs. In the reference implementation (`@cap/pipeline`), the pipeline also appends each produced event to the ledger as stages complete.
...
- **Chat platform boundary** — Untrusted or semi-trusted vendor payloads. Channel adapters are responsible for validation, deduplication hints, and mapping into CAP’s envelope. Downstream code should assume nothing about wire format beyond validated canonical events.
- **CAP core** — Middleware and delivery orchestration operate on canonical events only. Policy and routing see the same shaped facts the ledger stores, which keeps governance and audit aligned.
...
The repository ships **eleven** workspace packages. Arrows follow `dependencies` in each `package.json` (production dependencies only). `@cap/contract-harness` is the shared contract root (JSON Schema + Ajv validation). Nothing in `packages/` depends on `@cap/server`; the server is the runnable composition layer.

```mermaid
graph TD
  CH["@cap/contract-harness"]

  EL["@cap/event-ledger"]
  CWC["@cap/channel-web-chat"]
  CS["@cap/channel-slack"]
  MW["@cap/middleware"]
  DEL["@cap/delivery"]
  BH["@cap/backend-http"]
  BO["@cap/backend-openai"]
  PL["@cap/pipeline"]
  SR["@cap/server"]
  AC["@cap/adapter-conformance"]
...
| Package | Role |
|---------|------|
| `@cap/contract-harness` | Schema load and event validation |
| `@cap/event-ledger` | Append-only store (in-memory, SQLite) |
| `@cap/channel-web-chat` | Web chat ingress canonicalization |
| `@cap/channel-slack` | Slack ingress, send, streaming updates |
| `@cap/middleware` | Policy, routing, invocation event emission |
| `@cap/backend-http` | Generic HTTP backend invocation |
| `@cap/backend-openai` | OpenAI Chat Completions + streaming |
| `@cap/delivery` | Outbound orchestration, retry, DLQ behavior |
| `@cap/pipeline` | End-to-end orchestration and ledger appends |
| `@cap/server` | CLI / HTTP API runtime wiring Slack + OpenAI + SQLite |
| `@cap/adapter-conformance` | Shared tests for channel and backend adapters |

`@cap/pipeline` additionally lists `@cap/channel-slack` and `@cap/backend-openai` under **devDependencies** for integration tests; those edges are omitted from the graph above.
...
Both adapter families are covered by `@cap/adapter-conformance`, which reuses `@cap/contract-harness` validators to enforce the same schemas production code uses.
...
**New chat platforms** — Add a package (or module) that implements the channel ingress contract: map vendor webhooks or socket payloads to `message.received`, preserve correlation and conversation identifiers, and wire a `sendFn` (or equivalent) so delivery can post replies. Run the adapter against `@cap/adapter-conformance` channel fixtures before relying on it in production.
...
**Middleware policy and routing** — Replace or configure `PolicyFn` and routing rules inside `@cap/middleware` without changing channel or backend packages, as long as emitted events remain schema-valid and causally linked.
